...
obs_noise = zeros(size(all_rho));
obs_noise(:, 2, :) = randn(size(all_rho(:, 2, :)))*sigma;

all_rho = all_rho + obs_noise;
...
[RL_est_total, RL_Info_total] = ALS(A_mat, b_mat, rank_RL, 'X_true', RL, 'debugON', 0, 'NesterovON', 1, 'Nesterov_beta', 0.1);
fprintf('Finished in %.2f seconds \n', RL_Info_total.time)

RL_Info_total = error_analysis(RL_Info_total, RL_est_total, RL, 'displayON', 1, 'compute_dnorm', 1);

%% Learning in parallel for all first row blocks
[RL_est, RL_Info] = parallel_ALS_first_row_new(all_rho0, b_all, rank_RL, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, 'debugON', 0, 'displayON', 1, 'compute_dnorm', 1, 'Nesterov_beta', 0.5, 'num_retry', 3);


%% Learning the first row jointly
[RL_est_first_row, RL_est_first_row_Info] = ALS_first_row_joint_learning(all_rho0, b_all, rank_RL, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, 'num_retry', 0, 'debugON', 0, 'displayON', 1, 'compute_dnorm', 1, 'Nesterov_beta', 0.5);


%% Learning a random subset of the first row jointly
[RL_est_first_row_subset, RL_est_first_row_Info_subset] = ALS_first_row_joint_learning_random_subset(all_rho0, b_all, rank_RL, 'sub_ind_ratio', 0.4, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, ...
    'debugON', 0, 'plotON', 0, 'ALS_debugON', 0, 'displayON', 1, 'compute_dnorm', 1, 'Nesterov_beta', 0.6, 'num_retry', 3);
...
for i = 1:Num_samples
    ...
    fprintf('\nJoint ALS for the entire matrix...\n')
    [RL_est_total, RL_Info_total] = ALS(A_mat, b_mat, rank_RL, 'X_true', RL, 'debugON', 0);
    fprintf('Finished in %.2f seconds \n', RL_Info_total.time)

    RL_Info_total = error_analysis(RL_Info_total, RL_est_total, RL, 'displayON', 1, 'compute_dnorm', 1);

    %% Learning in parallel for all first row blocks
    [RL_est, RL_Info] = parallel_ALS_first_row_new(all_rho0, b_all, rank_RL, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, 'debugON', 0, 'displayON', 1, 'compute_dnorm', 1);


    %% Learning the first row jointly
    [RL_est_first_row, RL_est_first_row_Info] = ALS_first_row_joint_learning(all_rho0, b_all, rank_RL, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, 'num_retry', 3, 'debugON', 0, 'displayON', 1, 'compute_dnorm', 1);


    %% Learning a random subset of the first row jointly
    [RL_est_first_row_subset, RL_est_first_row_Info_subset] = ALS_first_row_joint_learning_random_subset(all_rho0, b_all, rank_RL, 'sub_ind_ratio', 0.2, 'X_true_sub_blocks', RL_sub_blocks, 'X_true', RL, 'debugON', 0, 'plotON', 0, 'ALS_debugON', 0, 'displayON', 1, 'compute_dnorm', 1);


    %% store the numbers
    all_ALS_N2{i} = RL_Info_total;
    all_ALS_P{i} = RL_Info;
    all_ALS_N{i} = RL_est_first_row_Info;
    all_ALS_I{i} = RL_est_first_row_Info_subset;
...
writetable(T, '/Users/quanjunlang/Documents/GitHub/Learning-operators-in-Lindblad-Master-Equation-Paper/5_1_2_Lindbladian_typical_data.csv')
