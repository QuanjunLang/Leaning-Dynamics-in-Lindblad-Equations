...
for i = 1:Num_M
    for k = 1:Num_samples
        ...
        try
            all_error(1, 1, i, k) = all_ALS_P{i, k}.rel_error_fro_X;
        catch 
            all_error(1, 1, i, k) = 100;
        end
        ...
        try
            all_error(2, 1, i, k) = all_ALS_N{i, k}.rel_error_fro_X;
        catch
            all_error(2, 1, i, k) = 100;
        end
        % all_error(3, 2, i, k) = all_ALS_N{i, k}.error_diamond;
        all_error(2, 2, i, k) = all_ALS_N{i, k}.time;

        try
            all_error(3, 1, i, k) = all_ALS_I{i, k}.rel_error_fro_X;
        catch
            all_error(3, 1, i, k) = 100;
        end
        % all_error(4, 2, i, k) = all_ALS_I{i, k}.error_diamond;
        all_error(3, 2, i, k) = all_ALS_I{i, k}.time;
    end
end
