...
# Using different observables to observe a given trajectory only once
    print('one observables for one traj')
    # Observables
    O = []
    for m in range(Num_traj):
        O.append(qutip.rand_herm(N, seed=rng, density = 0.8))
    ...
    for m in range(Num_traj):
        ...
        all_traj.append(np.array(curr_traj.expect[0]))
