...
result_prony = multi_mat_als(all_rho_prony_pair_data, trueInfo.r_true, 'true_para', trueInfo);
L_time = toc;
...
result_obs = multi_mat_als(all_rho_obs_pair_data, trueInfo.r_true, 'true_para', trueInfo);
%%
figure;
...
% L_est = ;
tic
decomp_result_prony = L_decomposition_hamiltonian_kossakowski(result_prony.L_est, sysInfo.p, trueInfo);
L_decomp_time = toc;

decomp_result_obs = L_decomposition_hamiltonian_kossakowski(result_obs.L_est, sysInfo.p, trueInfo);


%% (One stage) ALS Hamiltonian and Kossakowski 
tic
decomp_result_one_stage = ALS_hamiltonian_kossakowski(all_rho_prony_pair_data, sysInfo, sysInfo.p, trueInfo);
One_step_time = toc;
...
semilogy(decomp_result_one_stage.K_err, '-', 'DisplayName','Kossakowski matrix error', 'LineWidth',lnwd);
semilogy(decomp_result_one_stage.h_err, '-.', 'DisplayName','Hamiltonian coefficient error', 'LineWidth',lnwd);
set(gca, 'YScale', 'log')
legend()
